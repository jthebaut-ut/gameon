-- Expose existing sponsored placement radius targeting fields to the iOS
-- read-only lookup so the app can enforce regional eligibility before display.
...
COMMENT ON FUNCTION public.get_active_sponsored_placement(text, text, text, text, text) IS
  'Read-only active sponsored placement lookup for app surfaces. Hides paused/expired placements and returns optional radius targeting metadata for client-side regional eligibility.';
