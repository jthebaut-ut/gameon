-- Privacy-safe friend suggestions for fan profiles.
-- Suggestion signals are aggregated only; raw favorite/activity rows and precise locations are not returned.

CREATE INDEX IF NOT EXISTS idx_user_favorite_teams_team_user
  ON public.user_favorite_teams (team_id, user_id);

CREATE INDEX IF NOT EXISTS idx_venue_event_interests_event_email
  ON public.venue_event_interests (venue_event_id, user_email);

CREATE INDEX IF NOT EXISTS idx_pickup_game_requests_game_requester_status
  ON public.pickup_game_requests (pickup_game_id, requester_user_id, status);

CREATE INDEX IF NOT EXISTS idx_friendships_requester_addressee_status
  ON public.friendships (requester_id, addressee_id, status);

CREATE INDEX IF NOT EXISTS idx_friendships_addressee_requester_status
...
COMMENT ON FUNCTION public.get_profile_friend_suggestions(
  ...
  'Returns privacy-safe fan friend suggestions from aggregated shared favorite teams, shared venue-event interest, and approved pickup game participation. Excludes self, blocks, inactive/disabled/business profiles, accepted friends, and pending requests.';
