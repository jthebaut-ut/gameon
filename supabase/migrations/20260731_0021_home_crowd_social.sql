-- Home Crowd social card: set timestamp + venue fan count/avatars.

ALTER TABLE public.user_profiles
  ADD COLUMN IF NOT EXISTS home_crowd_set_at timestamptz;

COMMENT ON COLUMN public.user_profiles.home_crowd_set_at IS
  'When the fan selected their current home_crowd_venue_id.';

UPDATE public.user_profiles
SET home_crowd_set_at = created_at
WHERE home_crowd_venue_id IS NOT NULL
  AND home_crowd_set_at IS NULL;

CREATE OR REPLACE FUNCTION public.home_crowd_venue_summary(
  p_venue_id uuid,
...
BEGIN
  ...
  WHERE up.home_crowd_venue_id = p_venue_id
  ...
  SELECT COALESCE(
    jsonb_agg(
      jsonb_build_object(
        'user_id', ranked.id,
        'display_name', nullif(trim(coalesce(ranked.display_name, '')), ''),
        'avatar_url', nullif(trim(coalesce(ranked.avatar_thumbnail_url, ranked.avatar_url, '')), '')
      )
      ORDER BY ranked.sort_name, ranked.id
    ),
    '[]'::jsonb
  )
  INTO v_avatars
  FROM (
    ...
    WHERE up.home_crowd_venue_id = p_venue_id
      ...
      AND COALESCE(up.is_business_account, false) = false
      AND COALESCE(up.discoverable_by_fans, true) = true
      AND (p_exclude_user_id IS NULL OR up.id <> p_exclude_user_id)
    ORDER BY up.home_crowd_set_at DESC NULLS LAST, up.display_name NULLS LAST, up.id
    LIMIT 4
  ) ranked;

  RETURN v_venue
    || jsonb_build_object(
      'home_crowd_set_at', p_set_at,
      'home_crowd_fan_count', v_fan_count,
      'home_crowd_fan_avatars', v_avatars
    );
END;
$$;
...
BEGIN
  ...
  v_summary := public.home_crowd_venue_summary(p_venue_id, v_now, v_uid);
...
REVOKE ALL ON FUNCTION public.home_crowd_venue_summary(uuid, timestamptz, uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.home_crowd_venue_summary(uuid, timestamptz, uuid) TO authenticated;

-- Patch public identity RPC to return enriched home crowd payload.
