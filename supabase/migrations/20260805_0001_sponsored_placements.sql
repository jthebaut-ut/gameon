-- MVP sponsored venue placements.
-- Data/admin writes are service-role only. iOS reads active placements through the
-- read-only RPC below so normal venue listings and venue RLS stay unchanged.
...
CREATE TABLE IF NOT EXISTS public.sponsored_placements (
  ...
  subtitle text NULL,
  image_url text NULL,
  cta_label text NOT NULL DEFAULT 'View Venue',
  target_country text NULL,
  target_state text NULL,
  target_city text NULL,
  target_sport text NULL,
  starts_at timestamptz NOT NULL,
  ends_at timestamptz NOT NULL,
  status text NOT NULL DEFAULT 'active',
  created_at timestamptz NOT NULL DEFAULT now(),
  created_by uuid NULL,
  CONSTRAINT sponsored_placements_status_check
    CHECK (status IN ('active', 'paused', 'archived')),
  CONSTRAINT sponsored_placements_placement_key_check
    CHECK (placement_key IN ('profile_recommended_near_you')),
  CONSTRAINT sponsored_placements_date_window_check
    CHECK (ends_at > starts_at),
  CONSTRAINT sponsored_placements_title_not_blank_check
    CHECK (btrim(title) <> ''),
  CONSTRAINT sponsored_placements_cta_not_blank_check
    CHECK (btrim(cta_label) <> '')
);

COMMENT ON TABLE public.sponsored_placements IS
  'MVP first-party sponsored venue placements. Payments/ad serving are intentionally out of scope.';

COMMENT ON COLUMN public.sponsored_placements.placement_key IS
  'Current supported value: profile_recommended_near_you.';

CREATE INDEX IF NOT EXISTS idx_sponsored_placements_active_lookup
...
CREATE INDEX IF NOT EXISTS idx_sponsored_placements_venue_id
  ON public.sponsored_placements (venue_id);

CREATE INDEX IF NOT EXISTS idx_sponsored_placements_business_id
  ON public.sponsored_placements (business_id)
  WHERE business_id IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_sponsored_placements_targeting
  ON public.sponsored_placements (
    lower(target_country),
    lower(target_state),
    lower(target_city),
    lower(target_sport)
  );
...
DROP POLICY IF EXISTS sponsored_placements_service_role_manage
  ON public.sponsored_placements;

CREATE POLICY sponsored_placements_service_role_manage
  ON public.sponsored_placements
  FOR ALL
  USING (auth.role() = 'service_role')
  WITH CHECK (auth.role() = 'service_role');

REVOKE ALL ON public.sponsored_placements FROM PUBLIC;
GRANT SELECT, INSERT, UPDATE, DELETE ON public.sponsored_placements TO service_role;

CREATE OR REPLACE FUNCTION public.get_active_sponsored_placement(
...
COMMENT ON FUNCTION public.get_active_sponsored_placement(text, text, text, text, text) IS
  'Read-only active sponsored placement lookup for app surfaces. Hides paused/expired placements and inactive venues.';
