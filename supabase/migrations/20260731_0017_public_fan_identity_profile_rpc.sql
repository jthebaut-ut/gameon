-- Public-safe fan identity payload for profile previews (no email, no coordinates).
      ...
      (
        ...
        FROM (
          ...
          JOIN (
            ...
            ON target_friends.friend_id = viewer_friends.friend_id
          WHERE viewer_friends.friend_id NOT IN (v_viewer, v_target)
          LIMIT 4
        ) m
...
COMMENT ON FUNCTION public.get_public_fan_identity_profile(uuid) IS
  'Returns public-safe fan identity summary for profile previews; excludes undiscoverable, blocked, and business profiles.';
